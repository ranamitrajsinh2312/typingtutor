...
class _KeyboardLessonScreenState extends State<KeyboardLessonScreen> {
  ...
  // State variables - use grapheme clusters for proper Unicode
  late String test;
  late List<String> testChars; // Grapheme clusters
  List<Color> colors = [];
  ...
  void _loadLesson() {
    setState(() {
      ...
      // Use grapheme clusters for proper Unicode handling (Hindi/Gujarati)
      testChars = test.characters.toList();
  ...
  void _processKeyInput(RawKeyEvent event) {
    ...
    // Compare mapped character with expected character
    bool isCorrect;
    if (widget.languageCode == 'en') {
      // For English: case-insensitive comparison
      isCorrect = mappedChar.toLowerCase() == expectedChar.toLowerCase();
    } else {
      // For Indic scripts: compare grapheme clusters directly
      if (mappedChar == expectedChar) {
        isCorrect = true;
      } else if (mappedChar.characters.isNotEmpty &&
          expectedChar.characters.isNotEmpty) {
        isCorrect =
            mappedChar.characters.first == expectedChar.characters.first;
      } else {
        isCorrect = false;
      }
    }
                    ...
                    Expanded(
                      ...
                      child: Container(
                        ...
                        child: ModernKeyboardWidget(
                          ...
                          keyColors: keyColors,
                          currentChar:
                              count < testChars.length ? testChars[count] : '',
