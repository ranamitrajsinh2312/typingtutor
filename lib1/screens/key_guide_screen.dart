...
class _Level1State extends State<Level1> with TickerProviderStateMixin {
  ...
  double _getProgressPercentage() {
    if (_testChars.isEmpty) return 0;
    return (count / _testChars.length * 100).clamp(0, 100);
  }

  String _charAt(int index) {
    if (index < 0 || index >= _testChars.length) return '';
    return _testChars[index];
  }
  ...
  void _highlightWrongKey(String key) {
    ...
    setState(() {
      wrongKey = widget.languageCode == 'en' ? key.toUpperCase() : key;
  ...
  void _highlightCurrentKey(int charIndex, Color color) {
    if (!_isMounted || charIndex >= _testChars.length) return;
    String currentChar = _charAt(charIndex);
    if (currentChar == ' ') {
      c[3][0] = color;
      return;
    }
      ...
      for (int col = 0; col < keys[row].length; col++) {
        if (keys[row][col] == keyToHighlight ||
            (widget.languageCode == 'en' &&
                keys[row][col].toUpperCase() == keyToHighlight)) {
          c[row][col] = color;
          return;
        }
      }
  ...
  void _removeKeyHighlight(int charIndex) {
    if (!_isMounted || charIndex >= _testChars.length || charIndex < 0) return;
    String prevChar = _charAt(charIndex);
    if (prevChar == ' ') {
      c[3][0] = Colors.white;
      return;
    }
    ...
    for (int row = 0; row < keys.length - 1; row++) {
      for (int col = 0; col < keys[row].length; col++) {
        if (keys[row][col] == keyToRemove ||
            (widget.languageCode == 'en' &&
                keys[row][col].toUpperCase() == keyToRemove)) {
  ...
  String _getCurrentExpectedKey() {
    if (count >= _testChars.length) return '';
    return _charAt(count);
  }

  bool _isShiftRequired() {
    if (count >= _testChars.length) return false;
    String currentChar = _charAt(count);
    if (widget.languageCode == 'en') {
      return currentChar.toUpperCase() == currentChar &&
          currentChar.toLowerCase() != currentChar;
    }

    final shiftLayout = KeyboardLessonsData.getShiftKeyboardLayoutForLanguage(
      widget.languageCode,
    );
    for (final row in shiftLayout) {
  ...
  void _processKeyInput(RawKeyEvent event) {
    if (!_isMounted || event is! RawKeyDownEvent || count >= _testChars.length)
      return;

    _startGame();
    _clearWrongKeyHighlight();

    String expectedChar = _charAt(count);
    final key = event.logicalKey;
    isShiftPressed = event.isShiftPressed;
    _typingController?.setShift(isShiftPressed);
    ...
    final mappedChar = result.mappedChar;
    final isCorrect =
        _typingController?.compareCharacters(
          mappedChar,
          expectedChar,
          widget.languageCode,
        ) ??
  ...
  String whichImage(String c) {
    final finger = FingerMappingData.getFingerForChar(
      c == ' ' ? 'SPACE' : c,
      widget.languageCode,
    );
    if (finger == FingerType.leftPinky) {
      return 'assets/images/pinkey.png';
    } else if (finger == FingerType.leftRing) {
      return 'assets/images/ring.png';
    } else if (finger == FingerType.leftMiddle) {
      return 'assets/images/middle.png';
    } else if (finger == FingerType.leftIndex) {
      return 'assets/images/index.png';
    } else if (finger == FingerType.leftThumb) {
  ...
  String whichImageOnRight(String c) {
    ...
    if (finger == FingerType.rightPinky) {
      return 'assets/images/rightPinkey.png';
    } else if (finger == FingerType.rightThumb) {
      return 'assets/images/space.png';
    } else if (finger == FingerType.rightRing) {
      return 'assets/images/rightRing.png';
    } else if (finger == FingerType.rightMiddle) {
      return 'assets/images/rightMiddle.png';
    } else if (finger == FingerType.rightIndex) {
  ...
  void whichString(String lessonName, String languageCode) {
    ...
    setState(() {
      test = practiceText;
      _testChars = practiceText.characters.toList();
      colors = List.filled(_testChars.length, Colors.grey);
          ...
          if (showHandImages || width > 600)
            Flexible(
              ...
              child: Container(
                ...
                child: Image.asset(
                  whichImage(count < _testChars.length ? _charAt(count) : ' '),
                  errorBuilder:
                ...
                builder: (context, child) {
                  return Transform.translate(
                    ...
                    child: ModernKeyboardWidget(
                      keys: keys,
                      keyColors: c,
                      currentChar: count < _testChars.length ? _charAt(count) : '',
          ...
          if (showHandImages || width > 600)
            Flexible(
              ...
              child: Container(
                ...
                child: Image.asset(
                  whichImageOnRight(
                    count < _testChars.length ? _charAt(count) : ' ',
                  ),
                  errorBuilder:
                      (context, error, stackTrace) => Icon(
                        Icons.back_hand_outlined,
