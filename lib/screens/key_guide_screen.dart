...
// UI: /lib style (dark theme, stats bar, AppBar, modern keyboard)
// Hand Images: /liblatest style (left + right hand switch images)
// =========================================================================
...
class _Level1State extends State<Level1> with TickerProviderStateMixin {
  ...
  // ── Hand images from liblatest (via AppConstants) ─────────────────────────
  String _getLeftHandImage(String ch) => AppConstants.getLeftHandImage(ch);
  String _getRightHandImage(String ch) => AppConstants.getRightHandImage(ch);
  ...
  void _processKey(RawKeyEvent event) {
    ...
    if (isCorrect) {
      colors[count] = const Color(0xFF00C853);
      correct++;
      _pulseCtrl.forward().then((_) => _pulseCtrl.reverse());
    } else {
      colors[count] = const Color(0xFFFF1744);
      incorrect++;
  ...
  void _onComplete() {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      if (_isMounted && mounted) _showCompletionDialog(finalWpm, finalAcc);
    });
  }
  ...
  Widget _buildBody(Size size) {
    return Column(
      children: [
        _buildStatsBar(),
        Container(height: 80, child: _buildTextDisplay(size)),
        Expanded(
          child: Padding(
            padding: const EdgeInsets.only(bottom: 24.0),
            child: _buildKeyboardWithHands(),
          ),
        ),
      ],
    );
  ...
  // ── Keyboard + Hand Images (liblatest layout) ─────────────────────────────
  Widget _buildKeyboardWithHands() {
    final currentChar = count < test.length ? test[count] : ' ';

    return LayoutBuilder(
      builder: (context, constraints) {
        final availableHeight = constraints.maxHeight;
        final width = constraints.maxWidth;
        final showHands = ResponsiveHelper.shouldShowHandImages(
          context,
          availableHeight,
        );
        final handSize = ResponsiveHelper.getHandImageSize(context);
        final clampedHandSize = min(
          min(
            handSize * (width < 800 ? 0.85 : 1.0),
            width < 800 ? 110.0 : 160.0,
          ),
          availableHeight * 0.9,
        );
        ...
        return Padding(
          ...
          child: Row(
            ...
            children: [
              // Left Hand Image
              if (showHands || width > 600)
                Flexible(
                  flex: 2,
                  child: Container(
                    margin: EdgeInsets.symmetric(
                      horizontal: width < 800 ? 4 : 8,
                    ),
                    child: Image.asset(
                      _getLeftHandImage(currentChar),
                      height: clampedHandSize,
                      fit: BoxFit.contain,
                      errorBuilder:
                          (_, __, ___) => Icon(
                            Icons.back_hand_outlined,
                            size: width < 800 ? 36 : 52,
              ...
              // Keyboard
              Flexible(
                flex: (showHands || width > 600) ? 8 : 6,
                child: Container(
                  margin: EdgeInsets.symmetric(
                    horizontal: width < 800 ? 4 : 8,
                    vertical: width < 800 ? 4 : 8,
                  ),
              ...
              if (showHands || width > 600)
                Flexible(
                  ...
                  child: Container(
                    ...
                    child: Image.asset(
                      _getRightHandImage(currentChar),
                      height: clampedHandSize,
                      fit: BoxFit.contain,
                      errorBuilder:
                          (_, __, ___) => Icon(
...
// ── Key Guide Keyboard (lib dark-theme style) ─────────────────────────────────
...
class _KeyGuideKeyboard extends StatelessWidget {
  ...
  Widget _buildKey(BuildContext ctx, int ri, int ci, double maxW, double maxH) {
    ...
    double kh = maxH < 180 ? 28.0 : (maxH < 230 ? 34.0 : 42.0);
    // Ensure keys don't overflow vertically if maxH is extremely tight
    kh = min(kh, maxH * 0.18);
