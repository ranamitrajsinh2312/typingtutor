  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (ctx, cons) {
      ...
      return _KeyboardBody(
        keys: keys, keyColors: keyColors, wrongKey: wrongKey,
        currentChar: currentChar, languageCode: languageCode,
...
class _KeyboardBody extends StatelessWidget {
  ...
  const _KeyboardBody({
    required this.keys, required this.keyColors, required this.wrongKey,
    required this.currentChar, required this.languageCode,
    required this.indicToPhysical, required this.maxW, required this.maxH,
  });
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        ...
        padding: EdgeInsets.symmetric(horizontal: maxW * 0.012, vertical: maxH * 0.04),
        decoration: BoxDecoration(
          ...
          boxShadow: [
            ...
            BoxShadow(color: const Color(0xFF1A237E).withOpacity(0.2), blurRadius: 12, offset: const Offset(0, 2)),
          ],
  ...
  Widget _buildRow(BuildContext context, int ri) {
    return Row(
      ...
      children: List.generate(keys[ri].length, (ci) => _buildKey(context, ri, ci)),
    );
  }
  ...
  Widget _buildKey(BuildContext context, int ri, int ci) {
    ...
    final isWrong     = wrongKey != null &&
        (k == wrongKey || (k == 'SPACE' && (wrongKey?.toLowerCase() == 'space' || wrongKey == ' ')));
    final isHighlighted = ci < keyColors[ri].length &&
        (keyColors[ri][ci] == Colors.blue || keyColors[ri][ci] == Colors.blue.shade200 ||
    ...
    else if (isHomeRow && !_isIndic)           grad = [const Color(0xFFFFA726), const Color(0xFFEF6C00)];
    ...
    else if (isHomeRow && !_isIndic)                txtColor = const Color(0xFF1A0A00);
    else if (isSpecial)                             txtColor = const Color(0xFF8A9CC0);
    else                                            txtColor = const Color(0xFFD0D8EF);

    // ── Glow color ──────────────────────────────────────────────────────────
    final Color glowColor;
    if      (isWrong)                    glowColor = const Color(0xFFFF1744);
    else if (isCurrent || isHighlighted) glowColor = const Color(0xFF00E5FF);
    else if (isHomeRow && !_isIndic)     glowColor = const Color(0xFFFFA726);
    else                                 glowColor = Colors.transparent;

    final String? subLabel = _isIndic && !isSpecial && k != 'SPACE'
        ? indicToPhysical[k]?.toUpperCase() : null;
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          if (glowColor != Colors.transparent)
            BoxShadow(color: glowColor.withOpacity(0.6), blurRadius: 10, spreadRadius: 1),
          BoxShadow(color: Colors.black.withOpacity(0.3), blurRadius: 3, offset: const Offset(0, 2)),
          // Top highlight (3D effect)
          BoxShadow(color: Colors.white.withOpacity(0.06), blurRadius: 0,
              offset: const Offset(0, -1), spreadRadius: 0),
        ],
      ),
        ...
        FittedBox(fit: BoxFit.scaleDown,
          child: Text(_displayText(k),
            style: TextStyle(
              ...
              fontWeight: (isCurrent || isWrong) ? FontWeight.w900 : FontWeight.w600,
              fontFamily: (_isIndic && !isSpecial) ? 'Noto Sans Devanagari' : null,
  ...
  bool _isSpecialKey(String k) => ['TAB','CAPS','ENTER','SHIFT','SPACE'].contains(k);
  ...
  double _keyWidth(String k, double mw) {
    final base = mw / 16.5;
    switch (k) {
      case 'TAB':   return base * 1.55;
      case 'CAPS':  return base * 1.65;
      case 'ENTER': return base * 1.65;
      case 'SHIFT': return base * 2.1;
      case 'SPACE': return base * 6.8;
      default:      return base;
    }
  }
