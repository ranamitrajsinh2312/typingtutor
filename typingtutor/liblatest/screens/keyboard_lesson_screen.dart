...
class _KeyboardLessonScreenState extends State<KeyboardLessonScreen> {
  ...
  void _processKeyInput(RawKeyEvent event) {
    ...
    // Sync shift state with TypingController
    _typingController?.setShift(event.isShiftPressed);
    ...
    // Get the physical key label for mapping
    String physicalKey = event.logicalKey.keyLabel;

    // If keyLabel is empty, try character
    if (physicalKey.isEmpty && event.character != null) {
      physicalKey = event.character!;
    }

    // Skip if no valid key
    if (physicalKey.isEmpty) return;

    // Map physical key to language character
    String mappedChar =
        _typingController?.getMappedCharacter(physicalKey) ?? physicalKey;

    // Debug output for troubleshooting
    debugPrint(
      'Key: $physicalKey, Mapped: $mappedChar, Expected: $expectedChar, Lang: ${widget.languageCode}',
    );
